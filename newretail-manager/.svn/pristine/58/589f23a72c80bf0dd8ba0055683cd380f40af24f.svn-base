	...
	return{
		...
		initMainGrid:function(){
			...
			var buildGridObj={
				...
				colNames:[ '订单号','下单时间','配送人', '配送状态', '开始时间','结束时间'],//列名，列名的个数与colModel的个数要一致
				colModel:[
				{
...
//商家按状态+时间查询
function queryDeliverDetail(){
	console.log("11");
	$("#grid-table").jqGrid('clearGridData');
	var deliverStatus = $('#form-field-select-1').val();
	var queryTime = $("#id-date-picker-1").val();
	...
	$('#grid-table').jqGrid('setGridParam', {
		...
		mtype : 'post',
		url: '/deliver/queryDeliverDetail.do',		
	}).trigger('reloadGrid');
}

$(function(){
	deliver2.initMainGrid();
	$('#query').click(function(){queryDeliverDetail()});
})